...
reset(bool view_only)
{
    ...
    fp.buddha.real_steps = 5000;
    fp.buddha.imag_steps = 5000;

    f_color = FC_COLOR_RAW;
    fp.color = f_color;
#if !(defined(CUDA) || defined(OPENCL))
...
main(void)
{
    ...
    while (!WindowShouldClose()) // Detect window close button or ESC key
    {
        ...
        if (fp.mode == FC_MODE_LYAPUNOV)
            animate = false;
        if ((fp.mode == FC_MODE_MANDELBROT || fp.mode == FC_MODE_BUDDHA) && (animation == ANIMATION_DEFAULT || animation == ANIMATION_DEFAULT_ITERATIONS))
            animate = false;
        ...
        if (update) {
            // buddha mode doesn't necessarily overwrite every pixel --> reset color matrix
            if (fp.mode == FC_MODE_BUDDHA) {
                for (int h = 0; h < HEIGHT; ++h) {
                    for (int w = 0; w < WIDTH; ++w) {
                        *fractal_cmatrix_value(hc, h, w) = 0;
                    }
                }
            }
// Do the actual fractal computation
            ...
            float m = 255.f;
            if (fp.color == FC_COLOR_RAW) {
                m = fractal_cmatrix_max(hc);
            }
            for (int h = 0; h < HEIGHT; ++h) {
                for (int w = 0; w < WIDTH; ++w) {
                    Color c;
...
#else
                    if (fp.color == FC_COLOR_RAW) {
                        float value = (int)*fractal_cmatrix_value(hc, h, w);
                        c.r = c.g = c.b = value * 255.f / m;
                    } else {
                        uint8_t r, g, b;
        ...
        if (show_position) {
            sprintf(buf, "%.4f, %.4f", fp.x_start + (float)GetMouseX() / WIDTH * (fp.x_end - fp.x_start), fp.y_start + (float)GetMouseY() / HEIGHT * (fp.y_end - fp.y_start));
