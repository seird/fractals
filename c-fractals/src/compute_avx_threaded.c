    ...
    switch (fp->mode) {
        ...
        case FC_MODE_BUDDHA: {
            ...
            for (float cr = fp->x_start + x_step * targ->thread_id; cr < fp->x_end; cr += x_step * targ->num_threads) {
                for (float ci = fp->y_start; ci < fp->y_end; ci += y_step * VECFSIZE) {
                    __m256 c_real = _mm256_set1_ps(cr);
                    __m256 c_imag = _mm256_set_ps(ci, ci + y_step, ci + 2 * y_step, ci + 3 * y_step, ci + 4 * y_step, ci + 5 * y_step, ci + 6 * y_step, ci + 7 * y_step); // Little endian

                    buddha_avxf_get_trajectory(hc, fractal, &c_real, &c_imag, &RR, fp);
                }
            }
            break;
        }
...
fractal_avxf_get_colors_th(HCMATRIX hCmatrix, struct FractalProperties* fp, int num_threads)
{
    ...
    pthread_mutex_init(&mutex_buddha, NULL);
