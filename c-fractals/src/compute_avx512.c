...
__mmask16
fractal_avx512f_escape_magnitude_check(__m512* z_real, __m512* z_imag, __m512* RR)
{
                                 ...
                                 fractal_avx512_t fractal)
{
    ...
    for (int iteration = 0; iteration < max_iterations; ++iteration) {
        ...
        if (escaped_so_far_mask == 0xFFFF)
            break; // mask == 0b1111111111111111 --> all 16 pixels escaped
...
fractal_avx512f_get_colors(HCMATRIX hCmatrix, struct FractalProperties* fp)
{
    ...
    switch (fp->mode) {
            ...
            for (int h = 0; h < hc->height; ++h) {
                ...
                for (int w = 0; w < hc->width; w += VEC512FSIZE) {
                    ...
                    __m512 x_vec = _mm512_add_ps(
                      _mm512_set1_ps(x),
                      _mm512_set_ps(15 * x_step, 14 * x_step, 13 * x_step, 12 * x_step, 11 * x_step, 10 * x_step, 9 * x_step, 8 * x_step, 7 * x_step, 6 * x_step, 5 * x_step, 4 * x_step, 3 * x_step, 2 * x_step, x_step, 0)
                      // Little endian
                    );
        ...
        case FC_MODE_BUDDHA: {
            ...
            for (float cr = fp->x_start; cr < fp->x_end; cr += x_step) {
                for (float ci = fp->y_start; ci < fp->y_end; ci += y_step * VEC512FSIZE) {
                    __m512 c_real = _mm512_set1_ps(cr);
                    __m512 c_imag = _mm512_set_ps(ci, ci + y_step, ci + 2 * y_step, ci + 3 * y_step, ci + 4 * y_step, ci + 5 * y_step, ci + 6 * y_step, ci + 7 * y_step, ci + 8 * y_step, ci + 9 * y_step, ci + 10 * y_step, ci + 11 * y_step, ci + 12 * y_step, ci + 13 * y_step, ci + 14 * y_step, ci + 15 * y_step); // Little endian

                    buddha_avx512f_get_trajectory(hc, fractal, &c_real, &c_imag, &RR, fp);
                }
                printf("\r%.1f%%", 100 * (cr - fp->x_start) / (fp->x_end - fp->x_start));
                fflush(stdout);
            }
            break;
        }
