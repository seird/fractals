...
fractal_cmatrix_save(HCMATRIX hCmatrix, const char* filename, enum FC_Color color)
{
    if (color == FC_COLOR_RAW) {
        return fractal_cmatrix_save_raw(hCmatrix, filename);
    }
...
fractal_cmatrix_save_raw(HCMATRIX hCmatrix, const char* filename)
{
    ...
    for (int r = 0; r < hc->height; ++r) {
        for (int c = 0; c < hc->width; ++c) {
            int value = (int)*fractal_cmatrix_value(hCmatrix, r, c);
            data[r * (hc->width * 3) + (c * 3)] = value;
            data[r * (hc->width * 3) + (c * 3) + 1] = value;
            data[r * (hc->width * 3) + (c * 3) + 2] = value;
        }
    }
