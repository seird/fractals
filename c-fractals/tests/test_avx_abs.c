...
{   
    ...
    fractal_avxf_zabs2(&result_real_vec, &result_imag_vec, &z_real_vec, &z_imag_vec, &c_real_vec, &c_imag_vec);

    // Compute the julia_n result for n = 2
    __m256 result_real_n_vec, result_imag_n_vec;
    fractal_avxf_zabsn(&result_real_n_vec, &result_imag_n_vec, &z_real_n_vec, &z_imag_n_vec, &c_real_n_vec, &c_imag_n_vec, 2);
